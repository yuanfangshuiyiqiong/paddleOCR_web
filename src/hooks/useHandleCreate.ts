...
export default () => {
  ...
  const createImageElement = (url: string, shouldFitToCanvas = false) => {
    ...
    getImageSize(url).then(async ({ width, height }) => {
      ...
      if (shouldFitToCanvas) {
        ...
        const scaleY = (canvasHeight * 0.6) / height;
      ...
      const finalWidth = width * imageScale;
      const finalHeight = height * imageScale;

      const imageElement = await Image.fromURL(url, {}, {
        id: nanoid(10),
        angle: 0,
        left: centerPoint.x - finalWidth / 2,
        top: centerPoint.y - finalHeight / 2,
        scaleX: imageScale,
        scaleY: imageScale,
        hasControls: true,
        hasBorders: true,
