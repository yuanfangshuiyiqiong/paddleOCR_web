...
const handleFileSelect = async (event: Event) => {
  ...
  if (fileSuffix === 'pdf') {
    ...
    try {
      const { dataURL } = await renderPdfToDataURL(file)
      createImageElement(dataURL, true)
    } finally {
  ...
  if (res && res.data.code === 200) {
    ...
    // pdf/psd/cdr/ai 返回的是 svg，需要解析到 objects
    const svgList = res.data.svg
    if (svgList && svgList.length > 0) {
      const { objects } = await loadSVGFromString(svgList[0].svg)
      template.objects = [
        WorkSpaceDrawData,
        ...objects.slice(0, 1000).map((obj: FabricObject) =>
          obj.toObject(propertiesToInclude)
        ),
      ]
      template.width = svgList[0].width
      template.height = svgList[0].height
      template.workSpace = {
        fillType: 0,
        left: 0,
        top: 0,
