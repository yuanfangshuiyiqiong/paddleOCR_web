  ...
  <div>
    ...
    <div class="left-bottom-tabs">
      <div class="center-tabs">
        <div class="center-tab" :class="{ 'left-active': tab.key === poolType }" v-for="tab in topTabs" :key="tab.key"
...
const topTabs: TabItem[] = [
  ...
  { key: "annotation", label: "message.annotation", icon: "annotation", index: 7 },
  { key: "batchAnnotation", label: "message.batchAnnotation", icon: "batch-annotation", index: 8 },
...
const setBatchAnnotationTool = () => {
  const [ canvas ] = useCanvas();
  canvas.tool?.setTool('batchAnnotation')
  canvas.requestRenderAll()
};
...
const setPoolType = (tab: PoolType) => {
  ...
  if (tab === "annotation") {
    setShapeTool();
  } else if (tab === "batchAnnotation") {
    setBatchAnnotationTool();
  }
};
